...
The previously inspected Replogle K562 comparison is retained as external replication. Replogle RPE1 was reserved as the confirmatory falsification holdout in the original benchmark, with its target outcomes opened only after the CD4 features and folds were frozen. Both datasets are now previously inspected and are developmental-only for later extensions. Target-level outputs include every eligible target, and exclusions retain one reason per excluded target.
