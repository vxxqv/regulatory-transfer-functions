...
Do not regenerate the freeze when source hashes differ without documenting a new specification. Configuration and canonical cohort hashes use UTF-8 with LF newlines for portability. `canonical_cohort_sha256` identifies the tracked LF cohort; `original_crlf_cohort_sha256` retains the original CRLF byte hash. `finalize_statistics.py` reconciles the initial stored outputs after numerical review; it corrects matched-null direction and supplies prediction FDR without refitting models. The primary script now incorporates that reconciliation.
